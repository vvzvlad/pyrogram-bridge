# Рефакторинг пайплайна рендера: спецификация (v5)
...
База: ветка `refactor/render-pipeline` режется от **f9550d8**
(`feat/kurigram-2.2.23`); все line-refs спеки действительны для этого коммита.
Мёрж рефакторинга в main блокируется решением по kurigram-PR.
...
- База: `refactor/render-pipeline` от f9550d8. Один этап = один коммит
...
Фикстурный набор синтетических сообщений (naive-даты): одиночный пост,
media_group, time-based кластер, merged-группа с несколькими текстами, poll,
webpage-превью с фото и без, forward, реакции (обычные / custom / paid /
пустой список), текст с зачёркиванием (`<s>`/`<del>` через entities — без
такой фикстуры §3.1 в golden невидим).

Сценарии exclude_flags/exclude_text в golden-фикстуры НЕ входят: фильтры не
меняют байты выживших постов; их семантика (membership / regex) закрепляется
парой unit-тестов — дешевле и меньше площадь флаки-рисков.

**None-date фикстура в этап 0 НЕ входит**: текущий код падает на ней в
ДЕФОЛТНОМ пути (naive/aware TypeError в sort-ключе групп, см. §1) — снять
golden невозможно. Она добавляется в golden-набор коммитом этапа 4 со ссылкой
на §3.11–12 (это ДОБАВЛЕНИЕ фикстуры, а не обновление эталона).
